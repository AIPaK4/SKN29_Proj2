...
| 문제 유형 | 이진 분류 |
| 예측 대상 | 항공편의 지연 여부 |
| 지연 기준 | 조금이라도 지연 발생 시 지연으로 분류 (DelayCategory > 0) |
| 타겟 생성 기준 | `DelayCategory > 0`이면 지연(1), 그 외 정상(0) |
...
본 프로젝트의 타겟은 `DelayCategory`를 기준으로 생성하였다.

```text
DelayTarget = 1 if DelayCategory > 0 else 0
```

| 클래스 | 의미 |
|---|---|
| 0 | 지연 없음 (DelayCategory = 0) |
| 1 | 조금이라도 지연 발생 (DelayCategory > 0) |
...
이 때문에 단순 회귀로 지연 시간을 그대로 예측하기보다, **조금이라도 지연이 발생하는지 여부를 예측하는 이진 분류 문제**로 설정하는 것이 프로젝트 목적에 더 적합하다고 판단하였다.
...
| 1차 최종 피쳐 수 | 50개 | - |
| 최종 피쳐 수 | - | 38개 |
| 타겟 | `DelayCategory` | `DelayTarget` 이진 타겟 |
| 지연 기준 | 도착 지연 카테고리 | DelayCategory > 0 (조금이라도 지연 발생 시 지연) |
